...
namespace PatternsAndPractices
{
    public partial class AsyncPatternsAndPractices : Form
    {
        ...
        private async void WhenAll_Click(object sender, EventArgs e)
        {
            ...
            for (int nextIndex = 0; nextIndex < filenamesToCopy.Count; nextIndex++)
            {
                copyTasks.Add(CopyFileAsync(filenamesToCopy[nextIndex],
                    ...
                    null, null, true, new CancellationToken()));
            }
            string[] filesCopied = await Task.WhenAll(copyTasks);

            PlayAudioTrack(filesCopied.First(name => name.EndsWith(".mp3")),
                Path.Combine(AudioTempFolder.Text, @"Silents - Demo - cover.png"));
        }
        ...
        private async void WhenAnyFirstWins_Click(object sender, EventArgs e)
        {
            ...
            PlayAudioTrack(firstCopyTask.Result,
                Path.Combine(AudioSourceFolder.Text, @"aac\Silents - Demo - cover.png"));
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                UpdateProgess(progBar, 0, progDetails, "Copy " + Path.GetExtension(sourceFilepath) + " cancelled.");
                UpdateConsole(Console, "Copy cancelled : {0} ", Path.GetFileName(sourceFilepath));
